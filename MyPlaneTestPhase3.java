...
public class MyPlaneTestPhase3 {
    ...
    /******************************************************
     * Test Simulate MultiCity Trip
     *****************************************************/
    @Test
    public void testSimulateMultiCityTrip() {
        Plane p = new Plane();
        Random rand = new Random();
        int randomInteger = rand.nextInt(10) + 1;
        double currentHoursFlown = p.checkFlyingHours();
        boolean increased_hours = false;

        p.simulateMultiCityTrip(randomInteger);
        if( (currentHoursFlown + randomInteger) < p.checkFlyingHours()){
            increased_hours = true;
        }
        Assert.assertEquals("Since you refuel the plane after each city, the tank should be: ",
                TANK_CAPACITY, p.checkGasGauge(), 0.1);


        Assert.assertTrue("When you do a multi-city trip, your hours should increase " ,
                increased_hours);
    }
